...
type Phase = "settings" | "scanning" | "finalizing";

// ── Page principale ────────────────────────────────────────────────────────────

export default function PhotocopiePage() {
  const router = useRouter();
  const { kioskId: ctxKioskId, setJob } = useSession();
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setPhase("scanning");
      // Déclenchement immédiat de la première page
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      setJob(data.print_job_id, "Photocopie.pdf", data.pages, api.getPreviewUrl(data.print_job_id));
      router.push("/flow/config");
    } catch (err: any) {
      setError(err.message || "Erreur lors de la compilation du document.");
      setPhase("scanning");
    }
  };
  ...
  if (phase === "settings") {
    return (
      <div style={{ height: "100dvh", background: "#F5F6FA", display: "flex", flexDirection: "column", overflow: "hidden", fontFamily: "var(--font-sans, sans-serif)" }}>
        <FlowHeader title="Photocopie" step={1} total={3} onBack={handleCancel} />
        ...
        <div style={{ flex: 1, padding: "20px 18px", display: "flex", flexDirection: "column", gap: 14, overflow: "hidden" }}>
          ...
          <div style={{ background: "#FFFFFF", border: "0.5px solid #D8DBEE", borderRadius: 14, padding: "18px 16px", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, textAlign: "center", flex: 1, justifyContent: "center" }}>
            <div style={{ width: 72, height: 72, borderRadius: 18, background: "#EAF7F3", display: "flex", alignItems: "center", justifyContent: "center", color: "#16A38A" }}>
              <ScannerIcon />
            </div>
            <div>
              ...
              <p style={{ marginTop: 6, marginBottom: 0, marginLeft: "auto", marginRight: "auto", fontSize: 12, color: "#6B6E8C", lineHeight: 1.5, maxWidth: 220 }}>
                Placez votre feuille face vers le bas sur la vitre du scanner Canon G3030, puis refermez le capot.
              </p>
            </div>
          </div>
          ...
          <div style={{ background: "#FFFFFF", border: "0.5px solid #D8DBEE", borderRadius: 14, padding: "16px" }}>
            <p style={{ margin: "0 0 10px", fontSize: 11, fontWeight: 700, color: "#9B9DB8", textTransform: "uppercase", letterSpacing: "0.07em" }}>Mode de couleur</p>
              ...
              {(["nb", "couleur"] as const).map((mode) => (
                <button
                  ...
                  style={{
                    padding: "12px 0",
                    borderRadius: 10,
                    border: colorMode === mode ? "2px solid #16A38A" : "1.5px solid #D8DBEE",
                    background: colorMode === mode ? "#EAF7F3" : "#FFFFFF",
                    color: colorMode === mode ? "#0F6E56" : "#6B6E8C",
                    fontWeight: 700,
                    fontSize: 13,
                    cursor: "pointer",
                    display: "flex",
          ...
          {/* Tarif indicatif */}
          <div style={{ background: "#EEF0FF", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 13 }}>💡</span>
            <p style={{ margin: 0, fontSize: 11, color: "#3D3F7A", lineHeight: 1.4 }}>
              {colorMode === "nb"
                ? "Copie N&B : à partir de 50 FCFA/page"
                : "Copie couleur : à partir de 150 FCFA/page"}
            </p>
          </div>
        </div>
        ...
        <div style={{ padding: "12px 18px 28px", flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              borderRadius: 14,
              border: "none",
              background: scanning ? "#A8D5CC" : "#16A38A",
              color: "#FFFFFF",
              fontSize: 15,
              fontWeight: 700,
        ...
        <div style={{ flex: 1, padding: "16px 18px", display: "flex", flexDirection: "column", gap: 12, overflow: "hidden" }}>
          ...
          {scannedPages.length > 0 ? (
            ...
            <div style={{ flex: 1, background: "#FFFFFF", border: "0.5px solid #D8DBEE", borderRadius: 14, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, textAlign: "center", padding: 24 }}>
              <div style={{ width: 60, height: 60, borderRadius: 16, background: "#EAF7F3", display: "flex", alignItems: "center", justifyContent: "center", color: "#16A38A" }}>
                <ScannerIcon />
              </div>
              <div>
          ...
          <button
            ...
            style={{
              ...
              padding: "14px",
              borderRadius: 14,
              border: scanning ? "none" : "2px dashed #16A38A",
              background: scanning ? "#EAF7F3" : "#FFFFFF",
              color: scanning ? "#6B6E8C" : "#16A38A",
              fontSize: 14,
              fontWeight: 700,
              cursor: scanning ? "not-allowed" : "pointer",
              display: "flex",
        ...
        <div style={{ padding: "10px 18px 28px", flexShrink: 0, display: "flex", flexDirection: "column", gap: 8 }}>
          <button
            ...
            style={{
              ...
              borderRadius: 14,
              border: "none",
              background: scannedPages.length > 0 && !scanning ? "#16A38A" : "#D8DBEE",
          ...
          >
            <CheckCircleIcon />
            Finaliser ({scannedPages.length} page{scannedPages.length > 1 ? "s" : ""}) →
          </button>
          <button
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // PHASE : FINALISATION (spinner de compilation)
  // ─────────────────────────────────────────────────────────────────────────

  return (
    <div style={{ height: "100dvh", background: "#F5F6FA", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, fontFamily: "var(--font-sans, sans-serif)" }}>
      <div style={{ width: 72, height: 72, borderRadius: 18, background: "#EAF7F3", display: "flex", alignItems: "center", justifyContent: "center", color: "#16A38A" }}>
        <svg width={36} height={36} viewBox="0 0 24 24" fill="none" style={{ animation: "spin 1.2s linear infinite" }}>
          <circle cx="12" cy="12" r="10" stroke="rgba(22,163,138,0.25)" strokeWidth="4" />
          <path d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" fill="#16A38A" />
        </svg>
      </div>
      <div style={{ textAlign: "center" }}>
        <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#1B1B2E" }}>Compilation en cours…</p>
        <p style={{ margin: "6px 0 0", fontSize: 12, color: "#6B6E8C" }}>
          Assemblage de {scannedPages.length} page{scannedPages.length > 1 ? "s" : ""} en PDF
        </p>
      </div>
    </div>
  );
